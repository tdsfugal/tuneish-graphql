...
export default ({ i, cx, cy, r_outer, r_inner }) => {
  const angle = i * 30
  ...
  const x1 = cx + r_inner * s0
  const y1 = cy + r_inner * c0

  const x2 = cx + r_inner * s1
  const y2 = cy + r_inner * c1

  const x3 = cx + r_outer * s1
  const y3 = cy + r_outer * c1

  const x4 = cx + r_outer * s0
  const y4 = cy + r_outer * c0
